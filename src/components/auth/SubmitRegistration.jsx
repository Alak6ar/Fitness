...
const SubmitRegistration = () => {
  ...
  return (
    <form onSubmit={handleSubmit}><div className='flex justify-between items-center mb-14'>
    <h3 className='text-5xl'>OTP Verification</h3>
  </div>
    <div className='flex flex-col'>
      <div className='mb-11'>
        <label htmlFor="" className='block mb-3 text-base'>Enter code</label>
        <InputField {...inputs.codeVerify} />
        <div>
        {errors.confirmKey && touched.confirmKey && (
              <Validatior error={errors.confirmKey}/>
            )}
        </div>
      </div>
      <div>
      <div className='flex mb-11'>
      <p><span className='text-base'>Didn’t you receive the OTP? {time}</span> <button ref={resendBtnRef} type='button' onClick={resendOtpCode} className='text-blue-600 text-sm disabled:text-gray-500'>Resend OTP</button></p>
        </div>
      </div>
      <div>
